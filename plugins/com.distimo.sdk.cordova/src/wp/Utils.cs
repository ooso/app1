...
namespace Distimo
{
    internal sealed class Utils
    {
        ...
        {
            String threadString = (System.Threading.Thread.CurrentThread.IsBackground ? "BACKGROUND" : "MAIN") + " (" + System.Threading.Thread.CurrentThread.ManagedThreadId + ")";
            System.Diagnostics.Debug.WriteLine("[" + threadString + "] " + logString);
        }

        internal static object getApplicationSetting(String key)
        {
            return getApplicationSetting(key, null);
        }
        internal static object getApplicationSetting(String key, object defaultValue)
        {
            ...
            {
                //Exception while opening the application settings, return default value
                return defaultValue;
            } 
        ...
        internal static void setApplicationSetting(String key, object value)
        {
        ...
        internal static String Base64EncodeHexString(String hexString)
        {
            return Base64EncodeBytes(HexStringToBytes(hexString));
        }

        internal static String Base64EncodeBytes(byte[] bytes)
        {
        ...
        internal static byte[] HexStringToBytes(String hexString)
        {
            ...
            {
                string byteValue = hexString.Substring(2 * i, 2);
                result[i] = byte.Parse(byteValue, NumberStyles.HexNumber, CultureInfo.InvariantCulture);
            }
        ...
        internal static String deviceManufacturer()
        {
            if (isWP70())
            {
                String value = getLegacyValue("DeviceManufacturer");
                if (value != null)
                {
                    return value;
                }
            ...
            {
                String value = DeviceStatus.DeviceManufacturer;
                if (value != null)
                {
                    return value;
                }
        ...
        internal static String deviceModel()
        {
            if (isWP70())
            {
                String value = getLegacyValue("DeviceName");
                if (value != null)
                {
                    return value;
                }
            ...
            {
                String value = DeviceStatus.DeviceName;
                if (value != null)
                {
                    return value;
                }
        ...
        private static Boolean isWP70()
        {
            Version version = Environment.OSVersion.Version;
            return (version.Major == 7 && version.Minor == 0);
        }
